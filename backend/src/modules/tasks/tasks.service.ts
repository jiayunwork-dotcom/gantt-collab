...
export class TasksService {
  ...
  async create(projectId: string, dto: CreateTaskDto): Promise<Task> {
    ...
    return this.tasksRepository.findOne({ where: { id: saved.id } });
  }
  ...
  async update(projectId: string, taskId: string, dto: UpdateTaskDto): Promise<Task> {
    const task = await this.tasksRepository.findOne({
      where: { id: taskId, projectId },
    });
    if (!task) {
  ...
  async move(projectId: string, taskId: string, newParentId: string): Promise<Task> {
    const task = await this.tasksRepository.findOne({
      where: { id: taskId, projectId },
    });
    if (!task) {
